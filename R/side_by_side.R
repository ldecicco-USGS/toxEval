...
get_concentration_summary <- function(tox_list,
  ...
  chemical_summary <- chem_data %>%
    ...
    dplyr::left_join(dplyr::select(chem_site,
      ...
      by = "site") %>%
    dplyr::left_join(dplyr::select(chem_info, 
                                   CAS, Class),
                     by = "CAS")

  if (tox_names) {
    tox_names_key <- tox_chemicals %>%
      dplyr::select(CAS = Substance_CASRN, chnm = Substance_Name) %>%
      dplyr::filter(CAS %in% chem_info$CAS)

    chemical_summary <- chemical_summary %>%
      dplyr::left_join(tox_names_key, by = "CAS")

    if (any(is.na(chemical_summary$chnm))) {
      if ("Chemical" %in% names(chem_info)) {
        chemical_summary <- chemical_summary %>%
          dplyr::left_join(dplyr::select(chem_info, CAS, Chemical), 
                           by = "CAS")
      } else {
      ...
      chemical_summary <- dplyr::select(chemical_summary, -Chemical)
    }
  } else {
    if ("Chemical" %in% names(chem_info)) {
      chemical_summary <- chemical_summary %>%
